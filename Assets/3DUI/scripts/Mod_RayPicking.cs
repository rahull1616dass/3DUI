...
public class Mod_RayPicking : MonoBehaviour
{
    ...
    private ColliderToolScript ColissionScript;
    ...
    void Start()
    {
        ...
        ColissionScript = selectorObject.GetComponentInChildren<ColliderToolScript>();
    ...
    public void MakeOriginalColor()
    {
        ...
        var objectRenderer = ColissionScript.collidedObject.GetComponent<Renderer>();
        ...
        if (righHandDevice.isValid) // still connected?
        {
            if (righHandDevice.TryGetFeatureValue(CommonUsages.primary2DAxis, out Vector2 thumbstickAxis))
            {
                if (objectPickedUP != null) // already picked up an object?
                {
                    ...
                    {
                        objectPickedUP.transform.position += selectorObject.transform.TransformDirection(Vector3.up) *
                                                             translationIncrement * thumbstickAxis.y;
